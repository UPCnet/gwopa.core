...
def getProjectWaterOperators(project):
    result = []
...
def getProjectOtherOrganizations(project):
    result = []
    waterOperators = api.content.find(
...
def getTotalAssignedBudget(activities):
    total = 0
...
def getActivitiesWA(self, wa):
    ...
    data_year = self.context.gwopa_year_phases[int(self.context.project_year) - 1]
...
def getOutcomes(project):
    return api.content.find(
        portal_type=['OutcomeZONE'],
        context=project)


def getOutcomesCapacityWA(wa):
    return api.content.find(
        portal_type=['OutcomeCC'],
        context=wa)


def getTitleSpecific(specific):
    lang = getUserLang()
    if lang in ['es', 'fr']:
        return specific['title_specific_' + lang]
    else:
...
def updateReport(self):
    ...
    if refresh:
        ...
        logger.info('YYY - ' + str(self.context.save_data))
...
@grok.subscribe(IReport, IObjectAddedEvent)
def reportAdded(content, event):
